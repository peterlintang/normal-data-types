...
class ConcavePolygon
{
	...
	Vec2 tmp1 = ls1.direction();
	Vec2 tmp2 = ls2.direction();
    ...
    printf("%s: bestVert: %d, size: %d\n", __func__, bestVert, (int)vertsInCone.size());
    ...
    printf("%s: bestVert2: %d, size: %d\n", __func__, bestVert, (int)vertsInCone.size());
        ...
        {
		Vec2 vv = (ls1.direction() + ls2.direction()) * 1e+10;
...
public:
    ...
    {
	    printf("%s: vertex1: %d, vertex2: %d\n", __func__, vertex1, vertex2);
        if(vertex1 == vertex2 ||
           vertex2 == vertex1+1 ||
           vertex2 == vertex1-1)
            return;
	...
	printf("%s: left size: %d\n", __func__, (int)leftVerts.size());
	for (int i = 0; i < (int)leftVerts.size(); i++)
	{
		printf("i: %d, (%f, %f)\n", i, 
	...
	printf("%s: right size: %d\n", __func__, (int)rightVerts.size());
	for (int i = 0; i < (int)rightVerts.size(); i++)
	{
		printf("i: %d, (%f, %f)\n", i, 
    ...
    int getTotalSubPolys() const
    {
	int total = 0;
	printf("%s: subpolys %d\n", __func__, (int)subPolygons.size());
	if (subPolygons.size() == 0)
	{
		return 1;
	}
	...
	if (subPolygons.size() > 0)
	{
		total += getSubPolygon(0).getTotalSubPolys();
	}
	if (subPolygons.size() > 1)
	{
		total += getSubPolygon(1).getTotalSubPolys();
	}
