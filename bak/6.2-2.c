
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>

#define LEFT(i)		((i) * 2)
#define RIGHT(i)	((i) * 2 + 1)
#define PARENT(i)	((i) / 2)

/*
 * modify the property of min heap
 */
void min_heapify(int a[], int i, int size)
{
	int l;
	int r;
	int minimum;
	int tmp;

	l = LEFT(i);
	r = RIGHT(i);

	if (l < size && a[l] < a[i])
	{
		minimum = l;
	}
	else
	{
		minimum = i;
	}
	
	if (r < size && a[r] < a[minimum])
	{
		minimum = r;
	}

	if (i != minimum)
	{
		tmp = a[i];
		a[i] = a[minimum];
		a[minimum] = tmp;
		min_heapify(a, minimum, size);
	}

}

/*
 *use array to build max heap
 */
void build_min_heap(int a[], int size)
{
	int i;
	for (i = size / 2; i >= 0; i--)
	{
		min_heapify(a, i, size);
	}
}

/*
 * interfaces for max heap
 *
 * */
int min_heap_minimum(int a[], int size)
{
	return a[0];
}

int min_heap_extract_min(int a[], int *sizep)
{
	int min;
	int size;

	assert(sizep);

	size = *sizep;

	assert(size > 0);

	min = a[0];
	a[0] = a[size - 1];
	*sizep = size - 1;

	min_heapify(a, 0, size - 1);
	return min;
}

int min_heap_increase_key(int a[], int i, int key)
{
	int parent;

	if (a[i] > key)
	{
		return -1;
	}

	a[i] = key;
	parent = i >> 1; //parent = PARENT(i);
	while (a[i] < a[parent] && i > 0)
	{
		a[i] = a[parent];
		a[parent] = key;
		i = parent;
		parent = i >> 1; // parent = PARENT(i);
	}

	return 0;
}

int min_heap_insert(int a[], int *sizep, int key)
{

	a[*sizep] = +589238923;  // fu zheng wu qiong ;
	*sizep += 1;
	min_heap_increase_key(a, *sizep - 1, key);
}

/*
 * sort ..........
 */
void min_heap_sort(int a[], int size)
{
	int tmp;
	build_min_heap(a, size);
	while (size > 1)
	{
		tmp = a[0];
		a[0] = a[size - 1];
		a[size - 1] = tmp;
		size -= 1;
		min_heapify(a, 0, size);
	}
}


int main(int argc, char *argv[])
{
	int a[] = {1,2,8,7,14,4,16,3,9,10};
//	int a[] = {5, 3, 17, 10, 84, 19, 6, 22, 9};
#define ARRAY_SIZE(a)	(sizeof(a) / sizeof(a[0]))

//	build_max_heap(a, ARRAY_SIZE(a));
	min_heap_sort(a, ARRAY_SIZE(a));

	for (int i = 0; i < ARRAY_SIZE(a); i++)
	{
		fprintf(stdout, "a[%d] = %d\n", i, a[i]);
	}

	return 0;
}

