...
%[max_element_row,mmm]     = my_max(evcodal); 

[rows, cols] = size(eval); % 获取数组的行数和列数
maxValues = zeros(1, cols); % 初始化存储每列最大值的数组
maxValuesIndex = zeros(1, cols); % 初始化存储每列最大值的数组
 
for col = 1:cols
	maxValue = eval(1, col); % 假设第一行的值为最大值
  	maxValuesIndex(1)=1;
	for row = 2:rows
		if eval(row, col) > maxValue
			maxValue = eval(row, col); % 更新当前列的最大值
    			maxValuesIndex(col)=row;
		end
	end
	maxValues(col) = real(maxValue); % 存储最大值
end
max_element_row=maxValues;
%mmm=maxValuesIndex;


[~, max_column_index]   = max(max_element_row);

%fprintf('\n\n max_element_row  =\n'); disp(max_element_row);
%fprintf('\n\n max_column_index =\n'); disp(max_column_index);
%fprintf('\n\n mmm=\n'); disp(mmm);
%fprintf('\n\n mmmm=\n'); disp(mmmm);
