...
#include "../charpter-22/topological-sort.h"
 ...
 *
 */
int dag_shortest_paths(struct G *g, 
				struct weight_e *ws, int ws_num, 
				int s)
{
	...
	for (int j = 0; j < g->vs_num; j++)
	{
	    ...
	    for (cur = u->l.nil.next; cur != &(u->l.nil); cur = cur->next)
		{
			...
			if (v->d > u->d + w->w)
			{
				fprintf(stdout, "%s: v: %d, w: %d, weight: %d\n",
							__func__, v->v, u->v, w->w);
			}
		}
	}
...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout, "call dag-shortest-path for: %d\n", s);
	
	ret = dag_shortest_paths(g, ws, ws_num, s);
