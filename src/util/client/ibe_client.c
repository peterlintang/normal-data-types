...
static int process_event(unsigned char *buf, int len)
{
	int ret = 0;
	COMMAND_HEADER *pCmdHead = NULL;
...
static int ibe_dispatch(unsigned char *buf, int len)
{
	...
	printf("client pCmdHead->Command=%0x, len: %d, type: %d\n", pCmdHead->Command, len, pCmdHead->Type);
