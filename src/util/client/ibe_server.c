...
static int request_take_photo(int sock_fd, COMMAND_HEADER *cmdHead)
{
	int *my_data = NULL;

	my_data = (int *)(((unsigned char *)cmdHead) + COMMAND_HEADER_SIZE);
	fprintf(stdout, "%s my_data: %0x\n", __func__, *my_data);
	ibe_send_response(sock_fd, cmdHead->Command, 1, NULL, 0);
}
...
static int process_request(int sock_fd, unsigned char *buf, int len)
{
	...
	switch (pCmdHead->Command)
	{
		...
		case CMD_TAKE_PHOTO:
			request_take_photo(sock_fd, pCmdHead);
		break;
