...
AES_U8 in[16]  = {0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,  0x88,0x99,0xaa,0xbb,0xcc,0xdd,0xee,0xff};  //输入16字节（128位）明文
//AES_U8 encrypt_key[] = "1234567890ABCDEF";   //输入16字节（128位）密钥
void AES_encrypt_data(unsigned char *key, unsigned char *ptr_in, unsigned char *ptr_out)
{
    AES_U8  expKey[4 * Nc * (Nr + 1)];  
	...
	printf("\n");
#endif
    AES_Encrypt(ptr_in, expKey, ptr_out);  

}

void AES_decrypt_data(unsigned char *key, unsigned char *ptr_in, unsigned char *ptr_out)
{
    AES_U8  expKey[4 * Nc * (Nr + 1)];  
	AES_ExpandKey (key, expKey);
    AES_Decrypt (ptr_in, expKey, ptr_out); 
...
{
	unsigned char *ptr_in_tmp = ptr_in;
	unsigned char *ptr_out_tmp = ptr_out;

	if (ptr_in == NULL || ptr_out == NULL)
		return -1;
	if (len % 16 != 0)
		return -1;

	for (ptr_in_tmp = ptr_in; ptr_in_tmp < ptr_in + len; ptr_in_tmp += 16, ptr_out_tmp += 16)
	{
		AES_encrypt_data(key ,ptr_in_tmp, ptr_out_tmp); 
	}
...
int AES_decrypt_memory(unsigned char *key, unsigned char *ptr_in, unsigned char *ptr_out, int len)
{
	...
	for (ptr_in_tmp = ptr_in; ptr_in_tmp < ptr_in + len; ptr_in_tmp += 16, ptr_out_tmp += 16)
	{
		AES_decrypt_data(key, ptr_in_tmp, ptr_out_tmp); 
	}
...
int main(int argc, char *argv[])
{
	...
	unsigned char deout[16] = { 0 };
	
	if (argc != 3)
	{
		fprintf(stdout, "usage: %s [text] [secret]\n", argv[0]);
		return -1;
	}

	strcpy(in, argv[1]);
	strcpy(key, argv[2]);
	AES_encrypt_memory(key, in, out, 16);
	AES_decrypt_memory(key, out, deout, 16);

	fprintf(stdout, "encrypt: %s\n", out);
	fprintf(stdout, "decrypt: %s\n", deout);
	if (strncmp(in, deout, strlen(in)) != 0)
	{
