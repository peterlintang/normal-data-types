
#include <stdlib.h>
#include <string.h>
#include <limits.h>
#include <assert.h>

#include "text.h"

#define T Text_T

// < macros 278 >
#define IDX(i, len) ((i) <= 0 ? (i) + (len) : (i) - 1)

// < types 287 >
// < data 277 >
static char cset[] = 
"\000\001\002\003\004\005\006\007\010\011\012\013\014\015\016\017"
"\020\021\022\023\024\025\026\027\030\031\032\033\034\035\036\037"
"\040\041\042\043\044\045\046\047\050\051\052\053\054\055\056\057"
"\060\061\062\063\064\065\066\067\070\071\072\073\074\075\076\077"
"\100\101\102\103\104\105\106\107\110\111\112\113\114\115\116\117"
"\120\121\122\123\124\125\126\127\130\131\132\133\134\135\136\137"
"\140\141\142\143\144\145\146\147\150\151\152\153\154\155\156\157"
"\160\161\162\163\164\165\166\167\170\171\172\173\174\175\176\177"
"\200\201\202\221\204\205\206\207\210\211\212\213\214\215\216\217"
"\220\221\222\223\224\225\226\227\230\231\232\233\234\235\236\237"
"\240\241\242\243\244\245\246\247\250\251\252\253\254\255\256\257"
"\260\261\262\263\264\265\266\267\270\271\272\273\274\275\276\277"
"\300\301\302\331\304\305\306\307\310\311\312\313\314\315\316\317"
"\320\321\322\323\324\325\326\327\330\331\332\333\334\335\336\337"
"\340\341\342\343\344\345\346\347\350\351\352\353\354\355\356\357"
"\360\361\362\363\364\365\366\367\370\371\372\373\374\375\376\377";

const T Text_cset = { 256, cset };
const T Text_ascii = { 127, cset };
const T Text_ucase = { 26, cset + 'A' };
const T Text_lcase = { 26, cset + 'a' };
const T Text_digits = { 10, cset + '0' };
const T Text_null = { 0, cset };

// < static functions 286 >

// < functions 278 >
int MODULE_FUN_NAME(Text, pos)(T s, int i)
{
	assert(s.len >= 0 && s.str);

	i = IDX(i, s.len);

	assert(i >= 0 && i <= s.len);

	return i + 1;
}

T MODULE_FUN_NAME(Text, box)(const char *str, int len)
{
	T text;

	assert(str);
	assert(len >= 0);

	text.str = str;
	text.len = len;

	return text;
}

T MODULE_FUN_NAME(Text, sub)(T s, int i, int j)
{
	T text;

	assert(s.len >= 0 && s.str);

	i = IDX(i, s.len);
	j = IDX(j, s.len);
	if (i > j)
	{
		int t = i;

		i = j;
		j = t;
	}

	assert(i >= 0 && j <= s.len);

	text.len = j - i;
	text.str = s.str + i;

	return text;
}

T MODULE_FUN_NAME(Text, put)(const char *str)
{
	T text;

	assert(str);

	text.len = strlen(str);
	text.str = memcpy(calloc(1, text.len * sizeof(char)), str, text.len);

	return text;
}

char *MODULE_FUN_NAME(Text, get)(char *str, int size, T s)
{
	assert(s.len >= 0 && s.str);

	if (NULL == str)
	{
		str = (char *)calloc(1, (s.len + 1) * sizeof(char));
	}
	else
	{
		assert(size >= s.len + 1);
	}

	memcpy(str, s.str, s.len);
	str[s.len] = '\0';

	return str;
}

T MODULE_FUN_NAME(Text, dup)(T s, int n)
{
	T text;
	char *p = NULL;

	assert(s.len >= 0 && s.str);
	assert(n >= 0);

	if (0 == n || 0 == s.len)
	{
		return Text_null;
	}

	if (1 == n)
	{
		return s;
	}

	text.len = n * s.len;
	if (isatend(s, text.len - s.len))
	{
		text.str = s.str;
		p = (char *)calloc(1, (text.len - s.len) * sizeof(char));
		n--;
	}
	else
	{
		text.str = p = (char *)calloc(1, text.len * sizeof(char));
	}

	for (; n-- > 0; p += s.len)
	{
		memcpy(p, s.str, s.len);
	}

	return text;
}

T MODULE_FUN_NAME(Text, cat)(T s1, T s2)
{
	T text;

	assert(s1.len >= 0 && s1.str);
	assert(s2.len >= 0 && s2.str);

	if (s1.len == 0)
		return s2;
	if (s2.len == 0)
		return s1;

	if (s1.str + s1.len == s2.str)
	{
		s1.len += s2.len;
		return s1;
	}

	text.len = s1.len + s2.len;
	if (isatend(s1, s2.len))
	{
//		char *p = NULL;

		text.str = s1.str;
		memcpy(calloc(1, s2.len * sizeof(char)), s2.str, s2.len); // ???  TODO
	}
	else
	{
		char *p = NULL;
		text.str = p = (char *)calloc(1, (s1.len + s2.len) * sizeof(char));
		memcpy(p, s1.str, s1.len);
		memcpy(p + s1.len, s2.str, s2.len);
	}

	return text;
}

T MODULE_FUN_NAME(Text, reverse)(T s)
{
	T text;
	char *p = NULL;
	int i;

	assert(s.len >= 0 && s.str);

	if (s.len == 0)
	{
		return Text_null;
	}
	else if (s.len == 1)
	{
		return s;
	}
	else
	{
		i = s.len;
		text.len = s.len;

		p = (char *)calloc(1, s.len * sizeof(char));
		text.str = p;

		while (--i >= 0)
		{
			*p++ = s.str[i];
		}

		return text;
	}
}

T MODULE_FUN_NAME(Text, map)(T s, const T *from, const T *to)
{
	static char map[256];
	static int inited = 0;

	assert(s.len >= 0 && s.str);
	if (from && to)
	{
		int k;

		for (k = 0; k < sizeof(map); k++)
		{
			map[k] = k;
		}

		assert(from->len == to->len);
		for (k = 0; k < from->len; k++)
		{
			map[from->str[k]] = to->str[k];
		}
		inited = 1;
	}
	else
	{
		assert(NULL == from && NULL == to);
		assert(inited);
	}

	if (0 == s.len)
	{
		return Text_null;
	}
	else
	{
		T text;
		int i;
		char *p = NULL;

		text.len = s.len;
		text.str = p = (char *)calloc(1, s.len * sizeof(char));
		for (i = 0; i < s.len; i++)
		{
			*p++ = map[s.str[i]];
		}
		return text;
	}
}

int MODULE_FUN_NAME(Text, cmp)(T s1, T s2)
{
	int cond = 0;

	assert(s1.len >= 0 && s1.str);
	assert(s2.len >= 0 && s2.str);

	if (s1.str == s2.str)
	{
		return s1.len - s2.len;
	}
	else if (s1.len < s2.len)
	{
		cond = memcmp(s1.str, s2.str, s1.len);
		return cond == 0 ? -1 : cond;
	}
	else if (s1.len > s2.len)
	{
		cond = memcmp(s1.str, s2.str, s2.len);
		return cond == 0 ? 1 : cond;
	}
	else
	{
		return memcmp(s1.str, s2.str, s1.len);
	}
}









