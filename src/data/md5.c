...
md5_t *md5_finalizeMD5Transform (md5_ctx_t *context) 
{
    ...
    if (digest) {
        ...
        md5_updateMD5Transform (context, PADDING, padLen);

        /* Append length (before padding) */
        md5_updateMD5Transform (context, bits, 8);
...
int main(int argc, char *argv[])
{
	...
	ctx = md5_newMD5Transform();

	while ((ret = read(fd, buffer, 1000)) > 0)
	{
		md5_updateMD5Transform(ctx, buffer, ret);
	}
	md5 = md5_finalizeMD5Transform(ctx);
	md5_destroyMD5Transform(&ctx);
